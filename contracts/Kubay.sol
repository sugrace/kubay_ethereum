...
function highestBidderInfo(uint _productId) view public returns (address, uint, uint,address){
 ...
 return (product.highestBidder, product.highestBid, product.secondHighestBid,msg.sender);
}
...
uint _auctionEndTime, uint _startPrice ) public  {
   ...
   productIndex += 1;
   Product memory product = Product(productIndex, _game_name, _item_name, _game_money, _imageLink, _descLink, _auctionStartTime, _auctionEndTime,
        _startPrice, 0, 0, 0, 0, ProductStatus.Open);
...
function getProduct(uint _productId) view public returns (uint, string, string, uint, string, string, uint, uint, uint, ProductStatus) {
   Product memory product = stores[productIdInStore[_productId]][_productId];
   return (product.id, product.game_name, product.item_name, product.game_money, product.imageLink, product.descLink, product.auctionStartTime,
...
function refundAmountToBuyer(uint _productId) public {  Escrow(productEscrow[_productId]).refundAmountToBuyer(msg.sender);
}
